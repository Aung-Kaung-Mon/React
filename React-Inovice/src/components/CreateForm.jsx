...
const CreateForm = () => {
  ...
  const { products, createRecord } = useContext(generalContext);

  const handleBuyBtn = () => {
    if (quantityRef.current.value) {
      const currentProduct = products.find((product) => {
        return product.id == selectProductRef.current.value;
      });
      const newRecord = {
        id: Date.now(),
        ...
        cost: currentProduct.price * quantityRef.current.valueAsNumber,
      };

      createRecord(newRecord);
      selectProductRef.current.value = 1;
